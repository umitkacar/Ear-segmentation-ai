...
class TestEarPredictor:
    ...
    def test_predict_batch(self, mock_model_manager, test_batch_images):
        ...
        assert all(m.shape == img.shape[:2] for m, img in zip(masks, test_batch_images))
...
class TestStreamPredictor:
    ...
    def test_predict_frame_with_probability(self, mock_model_manager, test_image):
        ...
        mask, prob = predictor.predict_frame(test_image, return_probability=True)
