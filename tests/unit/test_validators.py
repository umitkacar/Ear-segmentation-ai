...
class TestImageArrayValidation:
    ...
    def test_invalid_type(self):
        ...
        with pytest.raises(InvalidInputError, match="Image must be numpy array"):
    ...
    def test_invalid_channels(self):
        ...
        with pytest.raises(InvalidInputError, match="Image must have 1, 3, or 4 channels"):
...
class TestMaskArrayValidation:
    ...
    def test_invalid_type(self):
        """Test validation with invalid type."""
        with pytest.raises(InvalidInputError, match="Mask must be numpy array"):
...
class TestDeviceStringValidation:
    ...
    def test_invalid_cuda_format(self):
        ...
        with pytest.raises(ValidationError, match="Invalid CUDA device format"):
...
class TestBatchSizeValidation:
    ...
    def test_invalid_type(self):
        ...
        with pytest.raises(ValidationError, match="Batch size must be integer"):
...
class TestCameraIdValidation:
    ...
    def test_valid_integer_camera_id(self):
        ...
        with patch('earsegmentationai.preprocessing.validators.cv2.VideoCapture', return_value=mock_cap):
    ...
    def test_negative_camera_id(self):
        ...
        with pytest.raises(ValidationError, match="Camera ID must be non-negative"):
    ...
    def test_invalid_camera_id_type(self):
        ...
        with pytest.raises(ValidationError, match="Camera ID must be integer or string"):
