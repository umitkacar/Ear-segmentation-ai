...
class TestVideoProcessorIntegration:
    ...
    def test_process_with_output(
        self, sample_video_file, temp_dir, mock_model_manager
    ):
    ...
    def test_process_camera(self, mock_capture, mock_model_manager):
        ...
        stats = processor.process(0, display=False)  # Camera ID
    ...
    def test_process_with_callback(
        self, sample_video_file, mock_model_manager
    ):
    ...
    def test_frame_skipping(self, sample_video_file, mock_model_manager):
        ...
        stats = processor.process(
            sample_video_file, display=False, max_frames=15
        )
    ...
    def test_stream_url(self, mock_capture, mock_model_manager):
        ...
        stats = processor.process("http://example.com/stream", display=False)
    ...
    def test_stats_calculation(self, sample_video_file, mock_model_manager):
        ...
        stats = processor.process(
            sample_video_file, display=False, max_frames=20
        )
    ...
    def test_display_overlay(self, sample_video_file, mock_model_manager):
        ...
        result = ProcessingResult(
            image=frame, mask=np.ones((480, 640), dtype=np.uint8), metadata={}
        )
