...
def benchmark_batch_processing(
    ...
    for batch_size in batch_sizes:
        ...
        for _ in tqdm(range(iterations), desc=f"Batch size {batch_size}"):
            start = time.time()
            processor.process(image_paths[:batch_size])
            end = time.time()
            times.append(end - start)
        ...
        results[f"batch_{batch_size}"] = {
            ...
            "speedup": float(batch_size / np.mean(times))
            / results.get("batch_1", {}).get("images_per_second", 1.0)
            if batch_size > 1
            else 1.0,
        }
...
def benchmark_image_sizes(
    image_path: Path, sizes: List[tuple], device: str = "cpu"
) -> Dict:
...
def benchmark_memory_usage(
    processor: ImageProcessor, image_path: Path
) -> Dict:
...
def main():
    ...
    for device in devices:
        ...
        # Memory usage
        print("\n3. Memory Usage")
        results[f"{device}_memory"] = benchmark_memory_usage(
            processor, test_image
        )

    # Image size benchmarks (CPU only for consistency)
    print("\n4. Image Size Benchmarks")
    sizes = [(320, 240), (640, 480), (1280, 720), (1920, 1080)]
    results["image_sizes"] = benchmark_image_sizes(
        test_image, sizes, device="cpu"
    )
