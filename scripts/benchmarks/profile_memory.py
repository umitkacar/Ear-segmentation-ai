...
def profile_single_image_memory(image_path: Path, device: str = "cpu") -> Dict:
    ...
    print(
        f"After initialization: {after_init['cpu_memory_mb']:.1f} MB "
    ...
    print(
        f"After processing: {after_process['cpu_memory_mb']:.1f} MB "
    ...
    print(
        f"After 10 iterations: {after_multiple['cpu_memory_mb']:.1f} MB "
...
def profile_batch_memory(
    image_paths: List[Path], batch_sizes: List[int], device: str = "cpu"
) -> Dict:
    ...
    for batch_size in batch_sizes:
        ...
        if torch.cuda.is_available() and device != "cpu":
            results[f"batch_{batch_size}"]["gpu_memory_mb"] = after.get(
                "gpu_memory_allocated_mb", 0
            )

        print(
            f"Batch size {batch_size}: CPU +{memory_increase:.1f} MB "
...
def profile_model_loading():
    ...
    print(
        f"After init: {after_init['cpu_memory_mb']:.1f} MB "
        f"(+{after_init['cpu_memory_mb'] - before['cpu_memory_mb']:.1f} MB)"
    )
    print(
        f"After load: {after_load['cpu_memory_mb']:.1f} MB "
