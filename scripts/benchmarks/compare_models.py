...
def benchmark_model_config(
    ...
    # Benchmark
    times = []
    for _ in tqdm(range(iterations), desc=f"{config.architecture}-{config.encoder}"):
        start = time.time()
...
def main():
    ...
    if "architectures" in results:
        architectures = sorted(results["architectures"], key=lambda x: x["fps"], reverse=True)
        print(f"{'Model':<30} {'FPS':<10} {'Params (M)':<12} {'Size (MB)':<10}")
        print("-" * 65)
        for arch in architectures[:5]:  # Top 5
            model_name = f"{arch['architecture']}-{arch['encoder']}"
            params_m = arch['total_params'] / 1e6
            print(f"{model_name:<30} {arch['fps']:<10.1f} {params_m:<12.1f} {arch['model_size_mb']:<10.1f}")
    ...
    if "input_sizes" in results:
        ...
        for size_result in results["input_sizes"]:
            print(f"{size_result['input_size']:<15} {size_result['fps']:<10.1f} {size_result['pixels']:<10}")
