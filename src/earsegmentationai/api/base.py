...
class BaseProcessor(ABC):
    ...
    def __init__(
        ...
        self.transform = ImageTransform(input_size=self.config.processing.input_size)
        ...
        logger.info(f"{self.__class__.__name__} initialized with device={self.config.processing.device}")
...
class BatchProcessingResult:
    ...
    def filter_by_detection(self, detected: bool = True) -> List[ProcessingResult]:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_ear_percentage": np.mean([r.ear_percentage for r in self.results if r.has_ear]) if any(r.has_ear for r in self.results) else 0.0,
        }