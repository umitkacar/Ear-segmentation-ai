...
def validate_mask_array(mask: np.ndarray, num_classes: int = 1) -> np.ndarray:
    ...
    if mask.ndim not in [2, 3]:
        raise InvalidInputError(
            f"Mask must be 2D or 3D array, got {mask.ndim}D"
        )
    ...
    if num_classes == 1:
        ...
        if not np.all(np.isin(unique_values, [0, 1])):
            logger.warning(
                f"Binary mask contains values other than 0 and 1: {unique_values}"
            )
    else:
        # Multi-class mask
...
def validate_batch_size(batch_size: int) -> int:
    ...
    if batch_size > 256:
        logger.warning(
            f"Large batch size: {batch_size}. This may cause memory issues."
        )
...
def validate_camera_id(camera_id: Union[int, str]) -> Union[int, str]:
    ...
    if isinstance(camera_id, int):
        if camera_id < 0:
            raise ValidationError(
                f"Camera ID must be non-negative, got {camera_id}"
            )
        ...
        if camera_id.startswith("/dev/"):
            ...
            camera_id.startswith("http") or camera_id.startswith("rtsp")
        ):
