...
def process_image(
    ...
    try:
        ...
        if save_results and output_dir is None:
            ...
            console.print(f"[yellow]No output directory specified, using: {output_dir}[/yellow]")
        ...
        if hasattr(result, "__len__"):  # Batch result
            ...
            console.print(f"Detection rate: [green]{result.detection_rate:.1f}%[/green]")
            ...
            if result.has_ear:
                ...
                if bbox:
                    console.print(f"Bounding box: x={bbox[0]}, y={bbox[1]}, w={bbox[2]}, h={bbox[3]}")
            else:
...
def process_video(
    ...
    try:
        ...
        console.print(f"Average FPS: [green]{stats['average_fps']:.1f}[/green]")
        console.print(f"Detection rate: [green]{stats['detection_rate']:.1f}%[/green]")
...
def benchmark(
    ...
    try:
        ...
        # Warmup
        if warmup > 0:
            console.print(f"[yellow]Warming up ({warmup} iterations)...[/yellow]")
            for _ in range(warmup):
                processor.process(image)
        
        # Benchmark
        console.print(f"[blue]Running benchmark ({iterations} iterations)...[/blue]")
