...
class MaskVisualizer:
    ...
    def visualize_comparison(
        ...
        if ground_truth is not None:
            ...
            diff_mask[(mask == 1) & (ground_truth == 0)] = 1  # False positive (red)
            diff_mask[(mask == 0) & (ground_truth == 1)] = 2  # False negative (blue)
            ...
            result = self._add_text(result, "Prediction", (image.shape[1] + 10, 30))
            result = self._add_text(result, "Ground Truth", (10, image.shape[0] + 30))
            result = self._add_text(result, "Difference", (image.shape[1] + 10, image.shape[0] + 30))
        else:
    ...
    def _add_contours(self, image: np.ndarray, mask: np.ndarray) -> np.ndarray:
        ...
        for class_idx in unique_classes:
            ...
            # Draw contours
            color = self.colors[class_idx] if class_idx < len(self.colors) else (255, 255, 255)
            cv2.drawContours(result, contours, -1, color, self.contour_thickness)
    ...
    def _add_legend(
        ...
        for i, class_idx in enumerate(class_indices):
            ...
            # Draw color box
            x = 10 + i * 150
            cv2.rectangle(result, (x, legend_y - legend_height), (x + 20, legend_y), color, -1)
