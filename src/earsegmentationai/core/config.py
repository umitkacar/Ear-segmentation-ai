...
class ModelConfig(BaseModel):
    ...
    name: str = Field(
        default="earsegmentation_model_v1_46.pth", description="Model filename"
    )
    url: str = Field(
    ...
    encoder_weights: str = Field(
        default="imagenet", description="Encoder weights"
    )
    ...
    expected_hash: Optional[str] = Field(
        default=None, description="Expected SHA256 hash of model file"
    )
...
class ProcessingConfig(BaseModel):
    ...
    input_size: tuple = Field(
        default=(480, 320), description="Input image size (width, height)"
    )
...
class VideoConfig(BaseModel):
    ...
    output_format: str = Field(
        default="avi", description="Output video format"
    )
    frame_size: tuple = Field(
        default=(640, 480), description="Output frame size"
    )
