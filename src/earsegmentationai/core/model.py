...
class ModelManager:
    ...
    def _verify_model(
        self, path: Path, expected_hash: Optional[str] = None
    ) -> bool:
        ...
        if expected_hash:
            ...
            if actual_hash != expected_hash:
                logger.error(
                    f"Model hash mismatch: {actual_hash} != {expected_hash}"
                )
                return False
        ...
        try:
            ...
            if model_class is None:
                raise InvalidModelError(
                    f"Unknown architecture: {model_config.architecture}"
                )
            ...
            logger.info(
                f"Created {model_config.architecture} model with {model_config.encoder_name} encoder"
            )
            return model
