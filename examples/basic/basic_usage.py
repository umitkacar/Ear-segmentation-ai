...
def process_single_image():
    ...
    if image_path.exists():
        result = processor.process(
            image_path, return_probability=True, return_visualization=True
        )
        ...
        if "visualization" in result.metadata:
            cv2.imshow(
                "Ear Segmentation Result", result.metadata["visualization"]
            )
            cv2.waitKey(0)
...
def process_image_directory():
    ...
    batch_result = processor.process(
        image_dir, save_results=True, output_dir=image_dir / "output"
    )
    ...
    for i, result in enumerate(batch_result):
        print(
            f"  Image {i}: Ear={'Yes' if result.has_ear else 'No'}, "
...
def process_webcam():
    ...
    def frame_callback(result):
        if result.has_ear:
            print(
                f"Frame {result.metadata.get('frame_number', 0)}: "
...
def process_with_custom_threshold():
    ...
    cv2.ellipse(
        test_image, (128, 128), (60, 40), 0, 0, 360, (255, 255, 255), -1
    )
    ...
    for threshold in thresholds:
        ...
        print(
            f"Threshold {threshold}: Ear area = {result.ear_percentage:.2f}%"
        )
...
def process_from_url():
    ...
    try:
        result = processor.process_from_url(
            image_url, return_visualization=True
        )
